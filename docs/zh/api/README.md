...
- `/api/v0.1/tasks`、`/api/v0.1/runs` —— 任务与运行相关接口；
- `/api/v0.1/submissions` —— 提交与结果上报相关接口；
- `/api/v0.1/evaluations`、`/api/v0.1/decisions` —— 评价与决策接口；
- `/api/v0.1/workers`、 `/api/v0.1/agents` —— Worker/Agent 管理及钱包相关接口。
...
```bash
curl -X POST "http://127.0.0.1:8000/api/v0.1/tasks" \
...
```bash
curl -X GET "http://127.0.0.1:8000/api/v0.1/tasks" \
...
```bash
curl -X POST "http://127.0.0.1:8000/api/v0.1/runs/<run_id>/submit" \
  -H "Content-Type: application/json" \
  -d '{
...
**查询余额 (Wallet State)**
```bash
curl -X GET "http://127.0.0.1:8000/api/v0.1/agents/<agent_id>/wallet" \
  -H "Accept: application/json"
```
...
针对复杂的长期任务，可以在 Run（运行实例）下拆分阶段性的 Dev Task：

```bash
curl -X POST "http://127.0.0.1:8000/api/v0.1/runs/<run_id>/dev-tasks" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "搭建项目基础架构",
    "description": "初始化目录结构和依赖",
    "priority": 1
  }'
```
